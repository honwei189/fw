...
class Fw
{
    ...
    {
        $this->http = flayer::bind("\\honwei189\\Flayer\\Http");
        ...
        if (isset($this->configuration['APP']) && is_value($this->configuration['APP'])) {
            $this->app = $this->configuration['APP'];
        }

        if (isset($_SESSION['APP']) && is_value($_SESSION['APP'])) {
    ...
    public function embed_file($key, $str)
    {
        $this->embed($key, $str);
    }

    public function embed_text($key, $str)
    {
